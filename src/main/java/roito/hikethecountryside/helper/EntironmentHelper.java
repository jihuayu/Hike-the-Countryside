package roito.hikethecountryside.helper;
...
public class EntironmentHelper
{
	...
	{
		if (ConfigMain.general.isDryingLimited)
		{
			return ConfigMain.craft.dryingBasicTime - (getDryingTemperatureLevel(temp) + getDryingRainfallLevel(rainfall)) * 100;
		}
		else
		{
			return ConfigMain.craft.dryingBasicTime;
		}
	}
	...
	{
		if (ConfigMain.general.isFermentationLimited)
		{
			return ConfigMain.craft.fermentationBasicTime - (getFermentationTemperatureLevel(temp) + getFermentationRainfallLevel(rainfall)) * 160;
		}
		else
		{
			return ConfigMain.craft.fermentationBasicTime;
		}
	}
